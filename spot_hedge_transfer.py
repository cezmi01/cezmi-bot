...
# --- Paribu yatırma açık mı? ----------------------------------------------
def is_paribu_deposit_open(coin: str) -> bool:
    """
    Paribu'da yatırma (deposit) açık mı?
    • Paribu API'sinden coin yatırma durumunu kontrol eder.
    """
    try:
        coin_u = coin.upper()
        
        # Paribu API'sinden coin bilgilerini al
        r = requests.get(
            "https://www.paribu.com/ticker",
            timeout=5
        )
        if r.status_code != 200:
            return True  # API hatası durumunda varsayılan olarak açık kabul et
        
        ticker_data = r.json()
        
        # Coin'in ticker'da olup olmadığını kontrol et
        # Paribu'da coin sembolleri genellikle büyük harfle ve TRY çiftiyle gelir
        coin_key = f"{coin_u}_TRY"
        
        if coin_key in ticker_data.get("data", {}):
            # Coin listede varsa yatırma açık kabul et
            return True
        
        # Coin listede yoksa da varsayılan olarak açık kabul et
        return True

    except Exception as err:
        send_telegram(f"❌ Paribu yatırma kontrol hatası: {err}")
...
def spot_alim_hedge_transfer(coin: str, target_exchange: str = "BTCTurk") -> bool:
    """Binance spot al – Binance futures short hedge – BTCTurk veya Paribu'ya transfer.

    • Binance tarafında `bin_coin` (alias'lı) kullanılır → BEAMX
    • Hedef borsa kontrollerinde orijinal `coin` kullanılır → BEAM
    • target_exchange: "BTCTurk" veya "Paribu"
    """
    ...
    # Hedef borsaya göre adres ve memo seç
    target_key = target_exchange.lower()  # "BTCTurk" -> "btcturk", "Paribu" -> "paribu"
    target_info = info.get(target_key, {})
    
    if not target_info:
        # Eski format desteği: direkt address ve memo alanları varsa kullan
        if "address" in info:
            address = info["address"]
            memo = info.get("memo")
        else:
            send_telegram(f"⛔ {coin}: {target_exchange} adresi coin_bilgileri.py içinde tanımlı değil")
            return False
    else:
    ...
    if not address:
        send_telegram(f"⛔ {coin}: {target_exchange} adresi boş")
        return False
    ...
    # Hedef borsaya göre yatırma kontrolü
    if target_exchange.lower() == "paribu":
        if not is_paribu_deposit_open(coin):
            send_telegram(f"❌ Paribu yatırma kapalı: {coin}")
            return False
    else:  # BTCTurk (varsayılan)
        if not is_btcturk_deposit_open(coin):
    ...
    send_telegram(f"✅ {coin} transferi {target_exchange} adresine gönderildi: {address[:10]}...")
    return True
