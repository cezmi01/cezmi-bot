...
def send_short_order_with_retry(symbol: str, coin: str, qty: float) -> None:
    ...
    qty = round_futures_qty(symbol, qty)
    qstr = f"{qty:.{qty_prec}f}"
...
def is_btcturk_deposit_open(coin: str) -> bool:
    ...
    try:
        ...
        if info.get("network", "").upper() == "SOL":
            search.insert(0, f"SPL{coin_u}")

        r = requests.get(
            "https://api.btcturk.com/api/v2/server/exchangeinfo", timeout=5
        ).json()
...
def _resolve_binance_symbols(coin: str, info: Dict[str, Any]) -> Tuple[str, str]:
    base = coin.upper()
    spot_coin = (
        info.get("binance_spot")
        or info.get("spot_alias")
        or info.get("spot")
        or info.get("fut")
        or base
    )
    fut_coin = (
        info.get("binance_futures")
        or info.get("futures_symbol")
        or _FUTURES_ALIAS_OVERRIDES.get(base)
        or info.get("fut")
        or spot_coin
    )
    return spot_coin.upper(), fut_coin.upper()
...
def spot_alim_hedge_transfer(coin: str) -> bool:
    ...
    bin_spot_coin, bin_fut_coin = _resolve_binance_symbols(coin, info)

    spot_symbol = f"{bin_spot_coin}USDT"
    fut_symbol = f"{bin_fut_coin}USDT"

    network = info["network"]
    address = info["address"]
    memo = info["memo"]

    if not is_withdraw_open(bin_spot_coin, network):
        send_telegram(f"❌ Binance çekim kapalı: {bin_spot_coin}/{network}")
        return False
    ...
    try:
        spot_price = get_price(spot_symbol)
    except Exception as exc:
        send_telegram(f"❌ Fiyat alınamadı {spot_symbol}: {exc}")
        return False

    target_qty = round_quantity(spot_symbol, TARGET_USDT / spot_price)
    filled_total = 0.0
    send_telegram(f"💸 {coin} alımı başlıyor → hedef {target_qty} (fiyat {spot_price})")

    while filled_total < target_qty:
        cur_price = get_price(spot_symbol)
        order_qty = round_quantity(spot_symbol, target_qty - filled_total)
        order_price = round_price(spot_symbol, cur_price)
        order = send_spot_order(spot_symbol, order_qty, order_price)
        if not order:
            break
        filled = wait_order_fill(order["orderId"], spot_symbol)
    ...
    send_short_order_with_retry(fut_symbol, coin, filled_total)

    send_withdrawal(bin_spot_coin, network, address, filled_total, memo)
    return True
