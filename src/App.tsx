...
function migrateWalletLinksFromOlderIfNeeded(): WalletLinks {
  ...
  const toHot4 = (hot?: string) =>
    hot
      ? ([hot, '', '', ''] as [string?, string?, string?, string?])
      : undefined
...
function getExplicitLinksForAsset(links: WalletLinks, exchangeId: string, asset: string) {
  ...
  const hotNormalized = [0, 1, 2, 3]
    .map((i) => normalizeUrl(hot[i as 0 | 1 | 2 | 3] ?? ''))
    .filter(Boolean)
...
export default function App() {
  ...
  const [walletLinks, setWalletLinks] = useState<WalletLinks>(() =>
    migrateWalletLinksFromOlderIfNeeded(),
  )
  ...
  const effectiveExchangeIds = useMemo(() => {
    const base = exchanges
      .filter((e) => visibleExchangeIds.includes(e.id))
      .map((e) => e.id)
    if (!hideEmptyColumns) return base
    return base.filter((id) =>
      filteredRows.some((r) => isConnected(r.cells[id] ?? null)),
    )
  ...
  return (
    <div className="h-[100dvh] w-screen overflow-hidden bg-[radial-gradient(1200px_600px_at_30%_-20%,rgba(56,189,248,0.15),transparent_60%),radial-gradient(900px_450px_at_90%_10%,rgba(34,197,94,0.10),transparent_55%)] p-4 md:p-6">
      <div className="flex h-full w-full flex-col gap-4">
        ...
        <Modal
          open={linksOpen}
          title="Borsa cüzdan linkleri (HOT / COLD)"
          onClose={() => setLinksOpen(false)}
        >
          <div className="muted text-sm">
            Buraya sadece web linklerini gir. Kaydedince tarayıcıya (localStorage)
            yazılır; HOT/COLD rozetlerine tıklayınca yeni sekmede açılır.
          </div>

          <div className="mt-4 grid gap-3">
            {exchanges.map((ex) => {
              const defHot =
                walletLinks[ex.id]?.[DEFAULT_ASSET_KEY]?.hot ?? ['', '', '', '']
                        ...
                        {[0, 1, 2, 3].map((i) => (
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              setWalletLinks((prev) => {
                                const existing =
                                  prev[ex.id]?.[DEFAULT_ASSET_KEY]?.hot ??
                                  ['', '', '', '']
                                const hot = [...existing] as [
                                  string?,
                                  string?,
                                  string?,
                                  string?,
                    ...
                    <table className="w-full min-w-[760px] border-separate border-spacing-0">
                      <thead>
                        <tr>
                          ...
                          <th className="px-2 py-2 text-left text-xs font-semibold text-slate-200">
                            HOT
                          </th>
                          <th className="px-2 py-2 text-left text-xs font-semibold text-slate-200">
                            COLD
                                  ...
                                  {[0, 1, 2, 3].map((i) => (
                                    <input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        setWalletLinks((prev) => {
                                          const existing =
                                            prev[ex.id]?.[asset]?.hot ?? [
                                              '',
                                              '',
                                              '',
                                              '',
                                            ]
                                          const nextHot = [...existing] as [
                                            string?,
                                            string?,
                                            string?,
                                            string?,
                ...
                {filteredRows.map((r) => (
                  <tr key={r.asset} className="hover:bg-white/[0.03]">
                    ...
                    {effectiveExchangeIds.map((id) => {
                      ...
                      const explicit = getExplicitLinksForAsset(
                        walletLinks,
                        id,
                        r.asset,
                      )
